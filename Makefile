...
IMAGE_TAG_BASE ?= ghcr.io/opentalon/opentalon-operator
...
# entire set of makefiles included in this invocation, looking for lines of
# the target-and-describing format and then formatting and displaying the
# formatted target-and-describing text.
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support.
	- $(CONTAINER_TOOL) buildx create --name opentalon-operator-builder
	$(CONTAINER_TOOL) buildx use opentalon-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform linux/arm64,linux/amd64 --tag ${IMG} -f Dockerfile .
	- $(CONTAINER_TOOL) buildx rm opentalon-operator-builder
...
sync-chart-crds: manifests ## Sync generated CRDs into Helm chart templates.
